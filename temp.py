...
from mindspeed.core.tensor_parallel.comm_group_api import A2AInnerNodeComm
from mindspeed.core.tensor_parallel.comm_group_api import A2AInterNodeComm
...
from mindspeed.core.transformer.moe.deep_ep_buffer import DeepEPBuffer
import torch.distributed as dist
from torch_npu import Buffer
...
class RouterFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, hidden_states, moe_layer):
        hidden_states_one = hidden_states.detach()
        hidden_states_one.requires_grad = True
        with torch.enable_grad():
            scores, indices = moe_layer.router(hidden_states_one)
        save_rf_tensors = [hidden_states_one, scores]
        ctx.save_for_backward(*save_rf_tensors)
        return hidden_states, scores.detach(), indices # check detach or not

    @staticmethod
    def backward(ctx, hidden_states_grad, *args):
        (hidden_states, scores) = ctx.saved_tensors
        scores.backward(args[0])
        result = hidden_states_grad + hidden_states.grad
        return hidden_states_grad + hidden_states.grad , None

class RouterFunc_same(torch.autograd.Function):
    @staticmethod
    def forward(ctx, hidden_states, moe_layer):
...
            return scores
        '''
        model_config = global_args.mm.model
        freeze_text_decoder = getattr(model_config.text_decoder, 'freeze', False)
        if not freeze_text_decoder:
            if not moe_hierarchical_alltoallv and global_args.n_shared_experts:
...
        # router
        router_input = hidden_states.detach()
        router_input.requires_grad = True
        if not recompute_instance.recompute:
            router_input = hidden_states.detach()
            router_input.requires_grad = True
            with torch.enable_grad():
                scores = router_func_test(router_input)

        else:
            with torch.enable_grad():
                scores = router_func_test(hidden_states)
        return hidden_states, scores.detach(), indices

    # @staticmethod
    # def backward(ctx, hidden_states_grad, *args):
    #     # print("debug moe layer backward", hidden_states_grad.sum())
    #     return hidden_states_grad, None

'''
forward:     input1 ->     o1       o1 -> o2
backward:    dinput  <-  dout      do1 <- do2
'''
class MoELayerOverlapAll2All(torch.autograd.Function):
    @staticmethod
    def forward(ctx, hidden_states, moe_layer: MoELayer, scores, indices):
...
        if args.enable_expert_relocation:
            # ctx.indices = moe_layer.expert_mapping[ctx.indices]
            indices = moe_layer.expert_mapping[indices]

        save_tensors.append(scores) #route_graph

        scores = scores.detach()
        scores.requires_grad = True
        save_tensors.append(scores) #detach_scores
...
        save_tensors.append(indices) #indices
...
                rs_share_experts_output = share_experts_output
                rs_shared_experts_handle = None
            (expert_output, mlp_bias), *expert_inputs_detach = forward_func(moe_layer.experts, (dispatched_input, tokens_per_expert, ctx))
...
            if use_deep_ep:
                save_tensors.append(expert_inputs_detach[0])
                dispatched_input.copy_(expert_output)
                output, mlp_bias = moe_layer.token_dispatcher.token_unpermutation(dispatched_input, mlp_bias, save_tensors)
            else:
                save_tensors.append(None)
...
        save_tensors.append(hidden_states) #share_experts_graph
...
            #print(f"output shape {output.shape} rs_share_experts_output shape {rs_share_experts_output.shape}",flush=True)
            output_sum = output + rs_share_experts_output
            # share_experts_output = torch.tensor(1, dtype=share_experts_output.dtype)
            output.untyped_storage().resize_(0)
            share_experts_output.untyped_storage().resize_(0)
        else:
            output_sum = output.detach()

        save_tensors.append(share_experts_output) #share_experts_graph
...
        print(f"debug moe layer forward", flush=True)
        return output_sum, mlp_bias

    @staticmethod
    def backward(ctx, *args):
        print(f"debug moe layer backward", flush=True)
        if use_deep_ep :
            print(f"debug moe layer use_deep_ep", flush=True)
            return MoELayerOverlapAll2All.backward_deepep(ctx, *args)
        else :
            return MoELayerOverlapAll2All.backward_src(ctx, *args)
    
    @staticmethod
...
                backward_ag_shared_handle = None
        inter_group = A2AInterNodeComm.get_comm_group()
        inner_group = A2AInnerNodeComm.get_comm_group()
        ep_group = parallel_state.get_expert_model_parallel_group()
        if moe_tp_extend_ep:
            ep_group = parallel_state.get_tensor_and_expert_parallel_group()
        
        output_backward_handle = None
        unpermute2_graph_backward_input = None
        if moe_hierarchical_alltoallv:
            unpermute2_graph_backward_input = args[0].view(-1, args[0].shape[-1])
            _, unpermute2_graph_backward_input, output_backward_handle = \
                async_all_gather(unpermute2_graph_backward_input, group=inter_group)
...
                        ep_group,
                    )
        print(f"trz backard args[0] shape {args[0].shape}",flush=True)
        if moe_hierarchical_alltoallv:
...
                    )
                    if not moe_tp_extend_ep and get_tensor_model_parallel_world_size() > 1 and ctx.moe_grouped_gemm and not moe_hierarchical_alltoallv:
...
            # ctx.scores_ep.untyped_storage().resize_(0)
...
                if moe_hierarchical_alltoallv:
                    _, probs_grad, probs_grad_handle = async_reduce_scatter(probs_grad, group=inter_group)
...
                # print(f"============line716, grad_output: {detach_scores.grad.min()}, {detach_scores.grad.max()}, {get_md5_for_tensor(detach_scores.grad)}", flush=True)
...
        
        if moe_hierarchical_alltoallv:
            return grad_output, None, detach_scores_grad, None
        else:
