...
def dispatch_preprocess(topk_idx: torch.Tensor, group: dist.ProcessGroup, buffer: Buffer, num_experts:int):
    ...
    return ref_num_tokens_per_expert, res_send_token_small, new_topk_idx
...
def alltoall_token_permutation_new(
    ...
    if use_deep_ep :
        return alltoall_token_permutation_new_deepep(self, hidden_states=hidden_states, probs=probs, indices=indices, shared_experts=shared_experts, save_tensors=save_tensors,shared_expert_gate=shared_expert_gate)
    else :
        return alltoall_token_permutation_new_src(self,hidden_states=hidden_states, probs=probs, indices=indices, shared_experts=shared_experts, save_tensors=save_tensors,shared_expert_gate=shared_expert_gate)
...
def alltoall_token_permutation_new_deepep(
        self, hidden_states: torch.Tensor, probs: torch.Tensor, indices: torch.Tensor, shared_experts, save_tensors, shared_expert_gate, moe_ctx=None
):
    moe_hierarchical_alltoallv = get_args().moe_hierarchical_alltoallv
    self.hidden_shape = hidden_states.shape
    probs_f16 = probs.to(torch.float32)
    self.probs = probs_f16
    self.indices = indices
    print(f"DEBUG: probs dtype: {probs.dtype}, shape: {probs.shape}, device: {probs.device}", flush=True)
    assert probs.dim() == 2, "Expected 2D tensor for probs"
    assert indices.dim() == 2, "Expected 2D tensor for indices"
    save_tensors.append(indices)
    save_tensors.append(probs_f16)
    ...
    tokens_per_expert, res_send_token_small, new_topk_idx = dispatch_preprocess(indices, num_of_expert, deepep_buffer, get_args().num_experts)

    self.send_token_small = res_send_token_small
    self.new_topk_idx = new_topk_idx
    save_tensors.append(res_send_token_small)
    save_tensors.append(tokens_per_expert)
    save_tensors.append(new_topk_idx)

    # save_tensors.append(hidden_states_ep if moe_hierarchical_alltoallv else hidden_states)
    if get_args().moe_experts_pipeline_degree and not moe_hierarchical_alltoallv:
        tokens_per_expert = tokens_per_expert.cpu()

    input_hidden_states = hidden_states.view(-1, self.hidden_shape[-1])
    #input_hidden_states = detach_tensor(input_hidden_states)
    return_values = deepep_buffer.dispatch(input_hidden_states, indices, tokens_per_expert, res_send_token_small, None, new_topk_idx)
    (
        expandx_out,
        dynamic_scales_out,
        expand_idx_out,
        put_offset,
        out_token_per_expert
    ) = return_values
    self.expand_idx_out = expand_idx_out
    ...
    if shared_experts is not None:
        ...
        if parallel_state.get_tensor_model_parallel_world_size() > 1 and shared_expert_gate is None:
            share_experts_graph, share_experts_output, rs_shared_experts_handle = async_reduce_scatter(
                ...
                event=None, stream=mindspore.runtime.default_stream())   # to_check
    ...
    self.expandx_out = expandx_out
    return share_experts_output, expandx_out, out_token_per_expert

def alltoall_token_permutation_new_src(
        self,
...
def alltoall_token_unpermutation_new(
    ...
    if use_deep_ep :
        return alltoall_token_unpermutation_new_deepep(self, hidden_states, bias, save_tensors)
    else :
        return alltoall_token_unpermutation_new_src(self, hidden_states, bias, save_tensors)

def alltoall_token_unpermutation_new_deepep(
        self, hidden_states, bias, save_tensors
):
    ep_group = parallel_state.get_expert_model_parallel_group()
    if get_args().moe_tp_extend_ep:
        ep_group = parallel_state.get_tensor_and_expert_parallel_group()
    #print(f"DEBUG: hidden_states: shape: {hidden_states.shape}", flush=True)
    #input_hidden_states = detach_tensor(hidden_states)
    deepep_buffer = DeepEPBuffer.get_deepep_buffer(ep_group)
    out_combine = deepep_buffer.combine(hidden_states, self.indices, self.expand_idx_out, self.put_offset, self.send_token_small, self.probs, self.new_topk_idx)
    out_combine = out_combine.view(self.hidden_shape)
    return out_combine, None

def alltoall_token_unpermutation_new_src(
        self, hidden_states, bias, save_tensors
):
